...
//CalculateDistanceMatrix takes a collection of DNA strings as input.
//It returns a distance matrix in which element (i,j)
//is the edit (Levenshtein) distance between strings i and j.
func CalculateDistanceMatrix(dnaStrings []string) DistanceMatrix {
	...
	for i := 0; i < numSamples; i++ {
		for j := i; j < numSamples; j++ {
			d := EditDistance(dnaStrings[i], dnaStrings[j])
			mtx[i][j] = d
			mtx[j][i] = d
